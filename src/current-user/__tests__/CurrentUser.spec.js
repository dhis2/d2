...
describe('CurrentUser', () => {
    ...
    describe('getUserGroups', () => {
        ...
        it('should be called with the userGroup ids', () => {
            ...
            expect(modelDefinitions.userGroup.list).toBeCalledWith({
                filter: [
                    'id:in:[vAvEltyXGbD,wl5cDMuUhmF,QYrzIjSfI8z,jvrEwEJ2yZn]',
                ],
                paging: false,
            })
        })
    ...
    describe('canCreate', () => {
        ...
        it('should return true for organisationUnit', () => {
            expect(
                currentUser.canCreate(modelDefinitions.organisationUnit)
            ).toBe(true)
        })
        ...
        it('should return false when the modelDefinition is in the noCreateAllowedFor list', () => {
            expect(
                currentUser.canCreate(modelDefinitions.categoryOptionCombo)
            ).toBe(false)
        })
    })
    ...
    describe('canDelete', () => {
        ...
        it('should return false for userGroup', () => {
            expect(currentUser.canDelete(modelDefinitions.userGroup)).toBe(
                false
            )
        })
        ...
        it('should return true for organisationUnit', () => {
            expect(
                currentUser.canDelete(modelDefinitions.organisationUnit)
            ).toBe(true)
        })
    })
    ...
    describe('canUpdate', () => {
        ...
        it('should return true for organisationUnitLevel', () => {
            expect(
                currentUser.canUpdate(modelDefinitions.organisationUnitLevel)
            ).toBe(true)
        })
    })
    ...
    describe('canCreatePublic', () => {
        ...
        it('should return false for userGroup', () => {
            expect(
                currentUser.canCreatePublic(modelDefinitions.userGroup)
            ).toBe(true)
        })

        it(
            'should return false for userGroup ' +
                'even when the user has the authority due to the presence in the ignore list',
            () => {
                noCreateAllowedFor.add('userGroup')

                expect(
                    currentUser.canCreatePrivate(modelDefinitions.userGroup)
                ).toBe(false)
            }
        )
    })
