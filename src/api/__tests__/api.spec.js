...
describe('Api', () => {
    ...
    beforeEach(() => {
        fetchMock = jest.fn().mockReturnValue(
            Promise.resolve({
                ok: true,
                text: () =>
                    Promise.resolve(fixtures.get('/api/schemas/dataElement'))
            })
        )
    ...
    it('should not be allowed to be called without new', () => {
        expect(() => Api(fetchMock)).toThrowError(
    ...
    describe('request()', () => {
        ...
        it('should report network errors', done => {
            fetchMock.mockReturnValueOnce(
                Promise.reject(new TypeError('Failed to fetch-o'))
            )
        ...
        it('should report 404 errors', done => {
            ...
            api.get('dataElements/404')
                .then(() => {
                    done(new Error('The request succeeded'))
                })
                .catch(err => {
    ...
    describe('get', () => {
        ...
        it('should not add a double slash to the url', () => {
            ...
            expect(fetchMock).toBeCalledWith(
                '/api/path/of/sorts/dataElements',
                baseFetchOptions
            )
        })
        ...
        it('should keep a full url if it is given as a base', () => {
            ...
            expect(fetchMock).toBeCalledWith(
                'http://localhost:8090/dhis/api/dataElements.json',
                baseFetchOptions
            )
        })
        ...
        it('should keep the the slashes if they are the first two characters', () => {
            ...
            expect(fetchMock).toBeCalledWith(
                '//localhost:8090/dhis/api/dataElements.json',
                baseFetchOptions
            )
        })
        ...
        it('should transfer data to the query string', () => {
            ...
            expect(fetchMock).toBeCalledWith(
                '/api/dataElements?fields=id%2Cname',
                baseFetchOptions
            )
        })
        ...
        it('should call the success resolve handler', done => {
            ...
            api.get('/api/dataElements', { fields: 'id,name' }).then(res => {
        ...
        it('should encode filters', () => {
            ...
            expect(fetchMock).toBeCalledWith(
                '/api/filterTest?filter=a:1&filter=b:2',
                baseFetchOptions
            )
        })
        ...
        it('should not double encode filter values', () => {
            ...
            expect(fetchMock).toBeCalledWith(
                '/api/filterTest?filter=name:eq:A%20%26%20B',
                baseFetchOptions
            )
        })
        ...
        it('should transfer complex filters to the query parameters', () => {
            ...
            expect(fetchMock.mock.calls[0][0]).toContain(
                '/api/complexQueryTest?'
            )
            expect(fetchMock.mock.calls[0][0]).toContain(
                'filter=id:eq:a0123456789&filter=name:ilike:Test'
            )
        })
    })
    ...
    describe('post', () => {
        ...
        it('should call the api the with the correct data', () => {
            api.post(
                ...
                fixtures.get('/singleUserOwnerFields')
            )
        ...
        it('should set remove the Content-Type header for form data', done => {
            ...
            api.post('form/data', data, {
                ...
                .then(() => {
                    expect(
                        fetchMock.mock.calls[0][1].headers.constructor.name
                    ).toBe('Headers')
                    expect(
                        fetchMock.mock.calls[0][1].headers.get('Content-Type')
                    ).toBeNull()
    ...
    describe('update', () => {
        ...
        it('should add the mergeMode param to the url when useMergeStrategy is passed', () => {
            ...
            expect(fetchMock).toBeCalledWith(
                '/api/some/fake/api/endpoint?mergeMode=REPLACE',
                fetchOptions
            )
        })

        it(
            'should add the mergeStrategy param to the url when useMergeStrategy is passed ' +
            ...
            () => {
                ...
                expect(fetchMock).toBeCalledWith(
                    '/api/some/fake/api/endpoint?mergeStrategy=REPLACE',
                    fetchOptions
                )
            }
        )
    ...
    describe('defaultHeaders', () => {
        ...
        it('should not use the defaultHeaders if specific header has been passed', () => {
            ...
            api.get('/me', undefined, {
                headers: { Authorization: 'Bearer ASDW212331sss' }
            })
