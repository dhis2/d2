...
describe('Yearly period', () => {
    describe('generateYearlyPeriodsUpToYear()', () => {
        ...
        it('should throw an error when passing a Date object', () => {
            expect(() =>
                generateYearlyPeriodsUpToYear(new Date())
            ).toThrowError()
        })
        ...
        it('should throw an error when the numberOfYears is not a positive integer', () => {
            expect(() =>
                generateYearlyPeriodsUpToYear(2017, 'a')
            ).toThrowError()
            expect(() =>
                generateYearlyPeriodsUpToYear(2017, 1.2)
            ).toThrowError()
            expect(() =>
                generateYearlyPeriodsUpToYear(2017, true)
            ).toThrowError()
            expect(() => generateYearlyPeriodsUpToYear(2017, -1)).toThrowError()
            expect(() => generateYearlyPeriodsUpToYear(2017, 0)).toThrowError()
            expect(() =>
                generateYearlyPeriodsUpToYear(2017, Infinity)
            ).toThrowError()
        })
        ...
        it('should use the current year when no year has been given', () => {
            expect(generateYearlyPeriodsUpToYear()).toEqual(
                generateYearlyPeriodsUpToYear(new Date().getFullYear())
            )
        })
    })
})
