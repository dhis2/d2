...
describe('Financial October period', () => {
    describe('generateFinancialOctoberPeriodsUpToYear()', () => {
        ...
        it('should throw an error when passing a Date object', () => {
            expect(() =>
                generateFinancialOctoberPeriodsUpToYear(new Date())
            ).toThrowError()
        })

        it('should generate 10 yearly periods when no numberOfYears was passed', () => {
            expect(generateFinancialOctoberPeriodsUpToYear(2017)).toHaveLength(
                10
            )
        })
        ...
        it('should respect the number of years parameter and generate the correct number of years', () => {
            const fiveYearlyPeriods = generateFinancialOctoberPeriodsUpToYear(
                2017,
                5
            )
        ...
        it('should throw an error when the numberOfYears is not a positive integer', () => {
            expect(() =>
                generateFinancialOctoberPeriodsUpToYear(2017, 'a')
            ).toThrowError()
            expect(() =>
                generateFinancialOctoberPeriodsUpToYear(2017, 1.2)
            ).toThrowError()
            expect(() =>
                generateFinancialOctoberPeriodsUpToYear(2017, true)
            ).toThrowError()
            expect(() =>
                generateFinancialOctoberPeriodsUpToYear(2017, -1)
            ).toThrowError()
            expect(() =>
                generateFinancialOctoberPeriodsUpToYear(2017, 0)
            ).toThrowError()
            expect(() =>
                generateFinancialOctoberPeriodsUpToYear(2017, Infinity)
            ).toThrowError()
        })
        ...
        it('should generate use the current year when no year has been given', () => {
            expect(generateFinancialOctoberPeriodsUpToYear()).toEqual(
