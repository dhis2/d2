...
import { validateIfValueIsInteger, formatAsISODate, getCurrentYear, addDays, getYYYYMM, getMonthNamesForLocale } from '../helpers';

export function generateFinancialOctoberPeriodsUpToYear(year = getCurrentYear(), numberOfYears = 10, locale = 'en') {
    validateIfValueIsInteger(year);
    if ((!isInteger(numberOfYears)) || numberOfYears < 1) { throw new Error('FinancialOctober generator parameter `numberOfYears` should be an integer larger than 0.'); }
