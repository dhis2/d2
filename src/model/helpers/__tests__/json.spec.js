...
describe('getJSONForProperties', () => {
    describe('for validationRule', () => {
        ...
        it('does fancy stuffs', () => {
            const model = validationRuleModelDefinition.create({
                id: 'a0123456789',
                organisationUnitLevels: [1, 2]
            })

            expect(
                getJSONForProperties(model, ['organisationUnitLevels'])
            ).toEqual({ organisationUnitLevels: [1, 2] })
        })
    })
    ...
    describe('for legendSet', () => {
        ...
        it('should embed the legends in the payload', () => {
            ...
            expect(
                getJSONForProperties(model, ['legends']).legends
            ).toHaveLength(6)
            expect(getJSONForProperties(model, ['legends']).legends).toEqual(
                legendSet.legends
            )
        })
        ...
        it('should not throw on userGroupAcceses', () => {
            ...
            expect(
                getJSONForProperties(model, ['userGroupAccesses'])
                    .userGroupAccesses
            ).toHaveLength(1)
        })
        ...
        it('should maintain the full structure of the userAccesses', () => {
            ...
            expect(
                getJSONForProperties(model, ['userAccesses']).userAccesses
            ).toEqual([
                {
                    access: 'rw------',
                    userUid: 'UgDpalMTGDr',
