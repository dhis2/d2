...
describe('ModelCollection', () => {
    ...
    describe('extension of Map', () => {
        ...
        beforeEach(() => {
            ...
            modelCollection = ModelCollection.create(
                mockModelDefinition,
                [],
                pagerObject
            )
        ...
        it('should get the entries', () => {
            expect(modelCollection.entries().next().value).toEqual([
                firstValue.id,
                firstValue
            ])
        })
        ...
        it('should run the forEach with the correct values', () => {
            ...
            expect(forEachFunc).toBeCalledWith(
                firstValue,
                'q2egwkkrfco',
                modelCollection.valuesContainerMap
            )
        })
        ...
        it('should get the entries', () => {
            expect(Array.from(modelCollection)[0]).toEqual([
                'q2egwkkrfco',
                firstValue
            ])
        })
    ...
    describe('class', () => {
        describe('create method', () => {
            ...
            it('should return an instance of the class', () => {
                expect(
                    ModelCollection.create(mockModelDefinition)
            ...
            it('should not be allowed to be called without new', () => {
                expect(() => ModelCollection()).toThrowError(
        ...
        describe('throwIfContainsOtherThanModelObjects', () => {
            it('should throw when one of the the passed values in the array is not a Model', () => {
                expect(() =>
                    ModelCollection.throwIfContainsOtherThanModelObjects([{}])
                ).toThrowError(
        ...
        describe('throwIfContainsModelWithoutUid', () => {
            it('should throw when the passed array contains a modelWithoutId', () => {
                expect(() =>
                    ModelCollection.throwIfContainsModelWithoutUid([
                        new Model(mockModelDefinition)
                    ])
                ).toThrowError(
            ...
            it('should accept models with valid UIDs', () => {
                ...
                expect(() =>
                    ModelCollection.throwIfContainsModelWithoutUid([model])
                ).not.toThrowError()
            })
        })
    })
    ...
    describe('instance', () => {
        ...
        it('should throw if being constructed with non Model values', () => {
            expect(
                () => new ModelCollection(modelDefinition, [1, 2, 3])
            ).toThrowError(
                'Values of a ModelCollection must be instances of Model'
            )
        })
        ...
        describe('add', () => {
            ...
            it('should not accept a number', () => {
                expect(() => modelCollection.add(1)).toThrowError(
                    'Values of a ModelCollection must be instances of Model'
                )
            })

            it('should not accept an empty object', () => {
                expect(() => modelCollection.add({})).toThrowError(
                    'Values of a ModelCollection must be instances of Model'
                )
            })
            ...
            it('should accept an object that was create with Model as subclass', () => {
                ...
                expect(() => modelCollection.add(myModel)).not.toThrowError(
                    'Values of a ModelCollection must be instances of Model'
                )
                expect(modelCollection.size).toBe(1)
            })
            ...
            it('should throw if the id is not available', () => {
                expect(() =>
                    modelCollection.add(new Model(mockModelDefinition))
                ).toThrowError(
                    'Can not add a Model without id to a ModelCollection'
                )
            })
