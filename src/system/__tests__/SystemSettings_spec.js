...
describe('settings.System', () => {
    ...
    it('should not be allowed to be called without new', () => {
        expect(() => SystemSettings()).toThrowError(
    ...
    describe('all', () => {
        beforeEach(() => {
            systemSettings.api.get.mockReturnValueOnce(
                Promise.resolve(settingsFixture)
            )
        })
    ...
    describe('get', () => {
        ...
        it('should return a Promise', () => {
            ...
            const result = systemSettings.get(
        ...
        it('should call the api to get the value', () => {
            ...
            expect(systemSettings.api.get.mock.calls[0][0]).toBe(
                'systemSettings/keyLastSuccessfulResourceTablesUpdate'
            )
        })
        ...
        it('should try to transform the response to json if possible', () => {
            systemSettings.api.get.mockReturnValueOnce(
                Promise.resolve('{"mydataKey": "myDataValue"}')
            )
        ...
        it('should also return a promise when serving cached values', () => {
            ...
            return systemSettings.all().then(() => {
                expect(
                    systemSettings.get('keyLastSuccessfulResourceTablesUpdate')
    ...
    describe('.set', () => {
        ...
        it('should POST to the API', () =>
            systemSettings.set('mySetting', 'my value').then(() => {
        ...
        it('should DELETE if the value is null or an empty string', () =>
            systemSettings.set('mySetting', '').then(() => {
                expect(mockApi.get).toHaveBeenCalledTimes(0)
        ...
        it('should not alter the value', () => {
            ...
            return systemSettings.set('mySetting', value).then(() => {
                expect(mockApi.get).toHaveBeenCalledTimes(0)
                expect(mockApi.post).toHaveBeenCalledTimes(1)
                expect(mockApi.delete).toHaveBeenCalledTimes(0)

                expect(mockApi.post.mock.calls[0][0]).toBe(
                    'systemSettings/mySetting'
                )
