...
describe('D2', () => {
    ...
    beforeEach(() => {
        ...
        ModelDefinitionsMock.getModelDefinitions = jest
            .fn()
            .mockReturnValue(new ModelDefinitionsMock())
    ...
    describe('config', () => {
        ...
        it('should call addStrings for the pre-init added strings', () => {
            ...
            d2.init(undefined, apiMock)
            return d2.getInstance().then(() => {
                expect(i18nMock.addStrings).toHaveBeenCalledWith([
                    'name',
                    'yes',
                ])
            })
    ...
    describe('getInstance', () => {
        ...
        it('should return the same instance on getInstance calls', () => {
            ...
            return Promise.all([d2.getInstance(), d2.getInstance()]).then(
    ...
    it('should throw an error when the passed config is not an object', () => {
        ...
        expect(shouldThrowOnString).toThrowError(
            'Expected Config parameter to have type object'
        )
        expect(shouldThrowOnFunction).toThrowError(
            'Expected Config parameter to have type object'
        )
    })
    ...
    it('should call the api', () =>
        d2.init({ baseUrl: '/dhis/api' }, apiMock).then(() => {
            const fields =
                'apiEndpoint,name,displayName,authorities,singular,plural,' +
    ...
    it('should log the error when schemas can not be requested', () => {
        ...
        return d2.init({ baseUrl: '/dhis/api' }, apiMock, loggerMock).then(
    ...
    it('should call the api for all startup calls', () => {
        ...
        return d2.init({ baseUrl: '/dhis/api' }, apiMock).then(() => {
    ...
    describe('with specific schema loading', () => {
        it('should have only loaded a single schema', () => {
            apiMock.get
                // First init round
                .mockReturnValueOnce(
                    Promise.resolve(fixtures.get('/api/schemas/user'))
                )
            d2.init(
                {
                    schemas: ['user'],
            ...
            return d2.getInstance().then(() => {
                expect(apiMock.get).toHaveBeenCalledWith('schemas/user', {
                    fields:
                        'apiEndpoint,name,displayName,authorities,singular,plural,shareable,metadata,klass,' +
    ...
    describe('getUserSettings', () => {
        ...
        it('should return an object with the uiLocale', () => {
            apiMock.get = jest
                .fn()
                .mockReturnValueOnce(
                    Promise.resolve(fixtures.get('/api/userSettings'))
                )
        ...
        it('should use the default base url when the set baseUrl is not valid', () => {
            ...
            return d2.getUserSettings(apiMock).then(() => {
    ...
    describe('getManifest', () => {
        ...
        it('should return a promise', () => {
            expect(d2.getManifest('manifest.webapp', apiMock)).toBeInstanceOf(
                Promise
            )
        })
        ...
        it('should request the manifest.webapp', () => {
            ...
            return d2.getManifest('manifest.webapp', apiMock).then(() => {
        ...
        it('should return the manifest.webapp object', () => {
            ...
            apiMock.get = jest
                .fn()
                .mockReturnValueOnce(Promise.resolve(expectedManifest))
